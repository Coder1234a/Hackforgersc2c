...
//
// gotcha: y grows DOWNWARD on a canvas. smaller y = higher up. detect.js
// says so too but it's caught all of us at least once.
...
// two faces off google fonts, w/ real fallback stacks behind them so it
// still looks like itself w/ the wifi off.
...
// knockVx = the shove from falling rock, kept apart from vx deliberately.
...
// a RUN = all four arenas back to back. name gets typed on the title
// screen, session only, so the end screen has someone to congratulate.
let playerName = "";
let runTotal = 0, runLog = [];

let revealAt = -999;                  // tick the score card went up

// every screen change fades and clears the DOM overlays - except the one
// that IS an overlay. the end screen used to come up UNDERNEATH the score
// card, which is why nobody could see it.
...
// something worth reporting happened. narrow the list + note the exact
// moment it became knowable, cos that's what the score is built on.
function logEvent(eventId) {
    if (!eventId || called) return;
    const before = liveSet.slice();
    const next = updateLiveSet(liveSet, eventId);
    if (next.length === 0) {
        // never supposed to happen. if it does, a detector is lying and
        // we'd rather shout than quietly show an empty list.
...
// back to spawn. same rule, same clock - dying shouldn't wipe what you
// worked out, and it shouldn't refund your time either.
function respawn(cause) {
    if (cause) {
        deaths++; deathCause = cause; goScreen("DEAD");
        // arena back to how it was drawn. a rough SHIFTING_PLATFORMS
        // shuffle, or every grass step already gone, can strand you w/
        // nothing in reach - and "stuck forever" isn't a puzzle. dying
        // costs you the clock + the moves, not the arena.
        resetLedges(level);
    ...
    // respawn zeroes your speed. if the release test is still armed from
    // before you died it reads that as STOPPED_PROMPTLY, which kills
...
// fresh run: totals cleared, back to arena one.
function startRun() {
    runTotal = 0; runLog = [];
    levelIndex = 0;
    newAttempt();
...
// arena 1 hides nothing, it's the control. finishing it doesn't ask you to
// name anything, it just walks you on. you can't notice a difference you
// were never shown.
...
// ---------- input ----------
//
// arrows or A/D move. SPACE, W or up jump. C = fundamentals. Q = new rule.
// R = back to spawn. ENTER = "yeah, continue" everywhere, and "lock it in"
// in the panel. that's the whole keyboard.
//
// the old C/L/N spread three unrelated jobs over three random letters and
// nobody could remember a single one.
...
document.addEventListener("keydown", function (e) {
    ...
    if (screen === "REVEAL") {
        // the ENTER that locks your answer in must NOT also skip the card
        // telling you if you were right. it did, and the whole reveal went
        // past in a single frame.
...
// the frame goes fullscreen, not the canvas. css letterboxes it so an
// ultrawide doesn't stretch the game.
...
// a LEDGE = anything thin. shelves, grass steps, cheese rinds, the runaway
// stub. land on top, pass straight through from underneath or the side -
// how every platformer has worked since about 1985.
//
// this is not decoration. before it, the small shelf sat directly over the
// mid shelf and the only jump that reached it had a THREE PIXEL launch
// window. nobody can play that.
//
// anything thick (cave roof, balcony) stays solid on all six sides, so
// REVERSE_GRAVITY still has a ceiling to stand on.
function isLedge(p) { return p.h <= 16; }

// were we clear of it when the frame began? w/out this, rising through a
// ledge and topping out inside it snaps you onto it mid-flight.
function wasClear(p, yBefore, g) {
    return g > 0 ? (yBefore + player.h <= p.y + 2)
                 : (yBefore >= p.y + p.h - 2);
}

// everything you can stand on this frame. ghosts count while invisible,
// which is the joke.
function solids() {
...
function update() {
    ...
    // a shove isn't walking, so the walk cap never sees it. rides on top,
    // bleeds off at 0.88/frame - that's what turns knock 22 into a real
    // 180px flight instead of a nudge.
    ...
    // four things make the direction reading a lie, so we shut up:
    //  - a moving ledge carries you whether you asked or not
    //  - a shove sends you the wrong way while you hold the key. looks
    //    exactly like inverted controls. isn't.
    //  - leftover speed under MOMENTUM does the same, so only read from a
    //    standstill where nothing but the controls can be moving you
    //  - rock. you can end a frame slightly inside a ledge's side, and
    //    pressing that way snaps you flush = a step BACKWARDS
    ...
    // let go - did we stop, or keep sliding? a wall counts as neither;
    // running into rock zeroes your speed whatever the rule is.
    ...
    // you have to have actually got going for "did you stop?" to mean a
    // thing. tap left then right under MOMENTUM and your speed passes clean
    // through zero - let go on that frame and it reads STOPPED_PROMPTLY,
    // which knocks out MOMENTUM while MOMENTUM is on.
    ...
    for (const p of ground) {
        ...
        // did we come down ONTO it, or bonk our head on it? "down" is
        // whichever way gravity points today. used to set onGround either
        // way, so jumping into a shelf glued you to its underside and you
        // could jump again off thin air.
        const landed = (player.vy > 0) === (g > 0);
        if (!landed) {
            if (isLedge(p)) continue;              // ledge: pass right through
            player.y = player.vy > 0 ? p.y - player.h : p.y + p.h;
            player.vy = 0;
            continue;                              // rock: bonk, don't stand
        }
        // only land on a ledge you were ABOVE when the frame began, else
        // rising through one snaps you onto it halfway up.
    ...
    // happened, and reporting it names NO_JUMP - contradicting whatever the
    // real rule was. blocked evidence is no evidence.
    ...
    // ---- cheesy chase ----
    // stand on a hole that isn't on the path and that's your run. no
    // warning, no second chance, straight back to the start.
    if (level.cheesed && landedPlat && landedPlat.safe) {
        // how far along you got. never says WHICH hole, only how many -
        // enough to learn from, not enough to skip the learning.
    ...
    // bullets. slow enough that walking into one is a choice.
    ...
    // falling rock. a hit never kills outright, it chucks you. near an edge
    ...
    for (const s of stals) {
        ...
        player.vy = -5;
        player.onGround = false;                   // lifted, not planted
        s.state = "gone"; s.gone = 60;
        puffDust(dust, s.x + s.w/2, s.y + s.h/2, 14, "#C99A63", 3.4);
        shove = 14;
    }

    // lasers just kill. no rule touches them, so never evidence.
    ...
    // no floor in these arenas. killY sits at the top of the dark so you die
    // where you can see it, not silently off the bottom.
...
// nobody reads "press enter" before they press something.
...
// six washes that mean nothing, deliberately. one per attempt, over
// whatever the arena looks like. you SEE the world shift the moment a new
// rule rolls, you just can't read WHICH one from it.
...
// draw decides nothing. reads the arena + the player, hands both to art.js.
// no camera transform either, so levels.js coords are screen coords - what
...
function draw() {
    ...
    // a ledge only gets drawn if the rule moved it - and we cover the hole
    // it came out of, else there's a ledge in the picture you fall through.
    ...
    // nothing drawn for a safe hole. traps get a whisper of shade.
    if (level.cheesed) {
    ...
    // attempt wash. "color" only touches hue + saturation, leaves brightness
    // alone, so the mood shifts w/out a single ledge edge getting harder to
    // read.
    ctx.save();
...
// every screen change fades in. cheap, and stops it feeling like slides.
...
// a real clock face. counts UP cos a leaderboard wants "solved in 8.4s" and
// a countdown can't give you that. under the timer rule the hand only moves
// when you do, and a frozen hand is way easier to spot than a bar draining
// slightly slower.
...
//
// nb what is NOT here: your best for the current rule. it used to be, and it
// handed the answer over - see the same number twice and you know you're on
// the same rule. lives on the score card now.
function drawHud(w) {
    // 34px, not 46. grassy falls has a door painted right up in the top
    // left and a fat bar swallowed it whole.
    const H = 34;

    // bar parts around any door painted at the very top of the frame.
    ...
    // arena name starts past x350 on purpose - see the notch above
    ...
    // pinned right so it lands sensibly whatever size the canvas is
    ...
    // shimmer last, so a door sat in a notch still breathes
    for (const d of [level.exit, level.trapDoor, level.resetDoor]) {
        if (d && d.y < H + 6) paintDoorGlow(ctx, d);
    }
}

// arena name for a couple of secs on the way in. makes moving between
// arenas feel like actually moving between arenas.
...
// after the first you know what you're looking at w/out reading it.
...
function drawTitle(w, h) {
    ...
    // name box. stored nowhere - it's just so the end screen has someone
    // to congratulate.
    ctx.fillStyle = "#6c7688"; ctx.font = uiFont("600 11px");
...
// end of a run. two columns: what you just did on the left, best runs on
// this machine on the right.
//
// the leaderboard wasn't invisible cos it was small - the score card was
// sat right on top of it. goScreen() clears the overlays now.
function drawEnd(w, h) {
    ...
    // ---- right: leaderboard, straight out of runs.js ----
    const rx = midX + 44;
    ctx.fillStyle = "#6c7688"; ctx.font = uiFont("600 11px");
...
// ---------- tutorial ----------
//
// six cards, each w/ a line pointing at the thing it's on about. only runs
// on the first cave. any key or click moves it on, so people who know the
// game skip it in six taps and people who don't get told what the clock,
// the rock and the call panel are for BEFORE they need them.
//
// at[] = the point the line touches. from[] = where the box wants to sit,
// clamped so a card can't end up half off the edge.
const TUTORIAL = [
...
// canvas has no word wrap, so here's one. splits on spaces, measures.
